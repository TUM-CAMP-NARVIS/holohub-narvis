...
Test to evaluate zenoh-network based data streaming
...
```bash
./holohub run tcn_receiver_test --local
```
...
```bash
# Build the container
./holohub build-container tcn_receiver_test

# Run the containerized application
./holohub run-container tcn_receiver_test
```
...
./holohub build-container tcn_receiver_test --base-image nvcr.io/nvidia/clara-holoscan/holoscan:v3.7.0-dgpu

# Run with specific GPU type
./holohub run-container tcn_receiver_test --gpu-type dgpu
```
